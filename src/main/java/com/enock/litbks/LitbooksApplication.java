package com.enock.litbks;

import com.enock.litbks.repositories.AuthorRepository;
import com.enock.litbks.repositories.BookRepository;
import com.enock.litbks.services.BookService;
import com.enock.litbks.ui.Main;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.CommandLineRunner;
import org.springframework.boot.SpringApplication;
import org.springframework.boot.autoconfigure.SpringBootApplication;

@SpringBootApplication
public class LitbooksApplication implements CommandLineRunner {

  @Autowired
  private BookService bookService;
  public static void main(String[] args) {
    SpringApplication.run(LitbooksApplication.class, args);
  }

  @Override
  public void run(String... args) throws Exception {
    Main main = new Main(bookService);
    main.menu();
  }
}

//62 +76 +